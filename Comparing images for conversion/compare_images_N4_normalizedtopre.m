function compare_images_N4_normalizedtopre (before,after,tags,finalfile)
...
[Fnonrecorded]=gradient(((R_N)+(R_N2)+(R_N3))/3);
slope=max(Frecorded);
...
[Frecordedvalue]=R;
[Fnonrecordedvalue]=(((R_N)+(R_N2)+(R_N3))/3);
...
save (['Compare Images time series normalized to pre condition for 4 neurons_' finalfile '.mat']); 
x=[0 10 30 80 180 330 880 1530 2380 3430];
plot(x,G,'m-o',x,R,'b-o',x,G_N,'g--',x,R_N,'r--',x,G_difference,'y--',x,R_difference,'k--',xpre,totalfluorescence,'c-*',x,G_N2,'g--',x,R_N2,'r--',x,G_N3,'g--',x,R_N3,'r--');
