function compare_images_N5_normalizedtopre (before,after,tags,finalfile)
...
close all
%% Choosing ROI, two choices will be made by the user: one for the recorded neuron one for the non recorded neuron. this ROI will be chosen to plot the data limited to that ROI.
...
[Fnonrecorded]=gradient(((R_N)+(R_N2)+(R_N3)+(R_N4))/4);
slope=max(Frecorded);
...
[Frecordedvalue]=R;
[Fnonrecordedvalue]=(((R_N)+(R_N2)+(R_N3)+(R_N4))/4);
...
save (['Compare Images time series normalized to pre condition for 5 neurons_' finalfile '.mat']); 
x=[0 10 30 80 180 330 880 1530 2380 3430];
plot(x,G,'m-o',x,R,'b-o',x,G_N,'g--',x,R_N,'r--',x,G_difference,'y--',x,R_difference,'k',xpre,totalfluorescence,'c-*',x,G_N2,'g--',x,R_N2,'r--',x,G_N3,'g--',x,R_N3,'r--',x,G_N4,'g--',x,R_N4,'r--');
title('After/Before conversion rate' );
xlabel('Cumulative Action Potentials Fired');
ylabel('Conversion Rate');
legend('Green fluorescence recorded neuron','Red fluorescence recorded neuron','Green fluorescence non recorded neurons','Red fluorescence non recorded neurons','Green Fluorescence trend line','Red Fluorescence trend line','Total(red+green) fluorescence level in each condition');
